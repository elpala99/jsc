In this document, the installation instruction for several packages are shown.
...
found. 
** FIXES
Fix right click, tested with gnome-tweaks (3.28.1)
#+begin_src bash
...
For *Nvidia* driver, instead of using =xbacklight=, install =light (1.2)=:
#+begin_src bash
